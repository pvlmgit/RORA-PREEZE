...
fun AppSelectionSheet(
    ...
    LaunchedEffect(filterOption) {
        if (filterOption == FilterOption.DISABLED) {
            refreshTick++
            installedApps = withContext(Dispatchers.IO) { AppUtil.getInstalledApps(context) }
        }
    }
    ...
    val groups = remember(installedApps, safety, searchQuery, sortOption, filterOption, refreshTick) {
        val filtered = installedApps.filter { app ->
            ...
            val matchesFilter = when (filterOption) {
                ...
                FilterOption.DISABLED -> !app.isEnabled
            }
                ...
                Row(
                    ...
                    IconButton(onClick = onDismiss) {
                        Icon(
                            ...
                            contentDescription = stringResource(R.string.select_apps_close)
                        )
                    }
                }
                SearchField(
...
private enum class FilterOption { ALL, USER, SYSTEM, DISABLED }
    ...
    Row(
        ...
        ControlChip(
            label = when (filterOption) {
                ...
                FilterOption.DISABLED -> stringResource(R.string.filter_disabled)
            },
