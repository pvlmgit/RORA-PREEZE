...
class FreezeViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun updateModeApps(context: Context, modeId: String, packageNames: List<String>) {
        ...
        if (activeModeId.value == modeId && removed.isNotEmpty()) {
            viewModelScope.launch {
                withContext(Dispatchers.IO) { FreezeManager.unfreezeApps(context, removed) }
            }
        }
    }
    ...
    fun activateMode(context: Context, modeId: String) {
        viewModelScope.launch {
            ...
            withContext(Dispatchers.IO) {
                val previousActiveId = activeModeId.value
                freezeModes.value.firstOrNull { it.id == previousActiveId }
                    ?.packageNames
                    ?.let { FreezeManager.unfreezeApps(context, it) }
